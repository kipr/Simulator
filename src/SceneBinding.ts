...
import Robotable from "./util/unused/Robotable";
...
import { preBuiltGeometries, preBuiltTemplates } from "./simulator/definitions/nodes";
import RobotBinding from './simulator/babylonBindings/RobotBinding';
import Robot from './state/State/Robot';
import AbstractRobot from './programming/AbstractRobot';
import WorkerInstance from "./programming/WorkerInstance";
import LocalizedString from './util/LocalizedString';
import ScriptManager from './simulator/ScriptManager';
...
class SceneBinding {
  ...
  private createRobot_ = async (id: string, node: Node.Robot): Promise<RobotBinding> => {
    ...
    observerObj.observer = this.bScene_.onAfterRenderObservable.add((data, state) => {
      ...
      robotBinding.origin = origin || ReferenceFramewUnits.IDENTITY;
    ...
    switch (node.type) {
      ...
      case Patch.Type.InnerChange: {
        ...
        switch (node.next.type) {
          ...
          case 'from-space-template': return this.updateFromTemplate_(id, node as Patch.InnerChange<Node.FromSpaceTemplate>, nextScene);
          default: {
  ...
  private createArcRotateCamera_ = (camera: Camera.ArcRotate): Babylon.ArcRotateCamera => {
    const ret = new Babylon.ArcRotateCamera('botcam', 10, 10, 10, Vector3wUnits.toBabylon(camera.target, 'centimeters'), this.bScene_);
  ...
  private createNoneCamera_ = (camera: Camera.None): Babylon.ArcRotateCamera => {
    const ret = new Babylon.ArcRotateCamera('botcam', 10, 10, 10, Vector3wUnits.toBabylon(Vector3wUnits.zero(), 'centimeters'), this.bScene_);
    ret.attachControl(this.bScene_.getEngine().getRenderingCanvas(), true);
  ...
  readonly setScene = async (scene: Scene, robots: Dict<Robot>) => {
    ...
    if (patch.gravity.type === Patch.Type.OuterChange) {
      this.bScene_.getPhysicsEngine().setGravity(Vector3wUnits.toBabylon(patch.gravity.next, 'centimeters'));
    }
