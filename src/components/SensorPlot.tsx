...
class SensorPlot extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    for (let i = 0; i < ticks.length; ++i) {
      const tick = ticks[i];
      secondTicks.push(
        <Tick theme={theme} x1={tick * pixelsPerSecond} y1={0} x2={tick * pixelsPerSecond} y2={viewportHeight} />
      );
    }
    ...
    return (
      <Container style={style} className={className} ref={this.bindRef_}>
        ...
        <Svg height={viewportHeight} width={viewportWidth} viewBox={`${x * pixelsPerSecond} ${0} ${svgWidth} ${viewportHeight}`} preserveAspectRatio='none'>
          {secondTicks}
