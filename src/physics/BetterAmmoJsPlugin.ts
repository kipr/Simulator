...
import { Quaternion as SimulatorQuaternion, Vector3 as SimulatorVector3 } from '../math';

import { ReferenceFrame } from '../math';
import BetterPhysicsJointData from './BetterPhysicsJointData';
 ...
 */
export class BetterAmmoJSPlugin implements IPhysicsEnginePlugin {
    /**
    ...
    public generatePhysicsBody(impostor: PhysicsImpostor) {
        ...
        if (impostor.isBodyInitRequired()) {
            ...
            if (impostor.soft) {
                ...
                colShape.setActivationState(BetterAmmoJSPlugin._DISABLE_DEACTIVATION_FLAG);
                ...
                if (mass === 0) {
                    body.setCollisionFlags(body.getCollisionFlags() | BetterAmmoJSPlugin._KINEMATIC_FLAG);
                    body.setActivationState(BetterAmmoJSPlugin._DISABLE_DEACTIVATION_FLAG);
                }
                ...
                if (impostor.type == PhysicsImpostor.NoImpostor && !colShape.getChildShape) {
                    body.setCollisionFlags(body.getCollisionFlags() | BetterAmmoJSPlugin._DISABLE_COLLISION_FLAG);
                }
    ...
    public generateJoint(impostorJoint: PhysicsImpostorJoint) {
        ...
        if (!BetterPhysicsJointData.isBetterPhysicsJointData(impostorJoint.joint.jointData)) {
            throw new Error("jointData must be of type BetterPhysicsJointData");
            return;
        }

        const { jointData } = impostorJoint.joint;

        const a = jointData.a || ReferenceFrame.IDENTITY;
        const b = jointData.b || ReferenceFrame.IDENTITY;

        const aOrientation = a.orientation || SimulatorQuaternion.IDENTITY;
        const bOrientation = b.orientation || SimulatorQuaternion.IDENTITY;

        const aPosition = a.position || SimulatorVector3.ZERO;
        const bPosition = b.position || SimulatorVector3.ZERO;

        const btAOrientation = new this.bjsAMMO.btQuaternion(
            aOrientation.x,
            aOrientation.y,
            aOrientation.z,
            aOrientation.w
        ...
        const btBOrientation = new this.bjsAMMO.btQuaternion(
            bOrientation.x,
            bOrientation.y,
            bOrientation.z,
            bOrientation.w
        ...
        const btAPosition = new this.bjsAMMO.btVector3(
            aPosition.x,
            aPosition.y,
            aPosition.z
        );
        
        const btBPosition = new this.bjsAMMO.btVector3(
            bPosition.x,
            bPosition.y,
            bPosition.z
        );

        const btAFrame = new this.bjsAMMO.btTransform(btAOrientation, btAPosition);
        const btBFrame = new this.bjsAMMO.btTransform(btBOrientation, btBPosition);

        let joint: any;
        switch (impostorJoint.joint.type) {
            case PhysicsJoint.HingeJoint: {
                joint = new this.bjsAMMO.btHingeConstraint(mainBody, connectedBody, btAFrame, btBFrame, true);
                break;
            }
            default:
                throw new Error("JointType not currently supported by the BetterAmmoJsPlugin");
        }

        this.world.addConstraint(joint, !jointData.collision);
    ...
    private _createShape(impostor: PhysicsImpostor, ignoreChildren = false) {
        ...
        let returnValue: any;
        const extendSize = impostor.getObjectExtents();
    ...
    public getRadius(impostor: PhysicsImpostor): number {
        const exntend = impostor.getObjectExtents();
        return exntend.x / 2;
    }
    ...
    public raycast(from: Vector3, to: Vector3) {
        ...
        return this._raycastResult as any;
    }
}