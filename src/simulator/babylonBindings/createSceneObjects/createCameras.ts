import {  ArcRotateCamera, Scene as babylonScene, Camera as babylCamera } from '@babylonjs/core';
...
export const createArcRotateCamera = (camera: Camera.ArcRotate, bScene_: babylonScene): ArcRotateCamera => {
...
export const createNoneCamera = (camera: Camera.None, bScene_: babylonScene): ArcRotateCamera => {
  const ret = new ArcRotateCamera('botcam', 10, 10, 10, Vector3wUnits.toBabylon(Vector3wUnits.zero(), 'centimeters'), bScene_);
  ret.attachControl(bScene_.getEngine().getRenderingCanvas(), true);
  return ret;
};

export const createCamera = (camera: Camera, bScene_: babylonScene): babylCamera => {
  switch (camera.type) {
