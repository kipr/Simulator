import { ShadowGenerator,  IShadowLight, PointLight, SpotLight, DirectionalLight,
  Scene as babylonScene } from '@babylonjs/core';
...
export const createDirectionalLight = (id: string, node: Node.DirectionalLight, bScene_: babylonScene, shadowGenerators_: Dict<ShadowGenerator>): DirectionalLight => {
...
export const createSpotLight = (id: string, node: Node.SpotLight, bScene_: babylonScene, shadowGenerators_: Dict<ShadowGenerator>): SpotLight => {
...
export const createPointLight = (id: string, node: Node.PointLight, bScene_: babylonScene, shadowGenerators_: Dict<ShadowGenerator>): PointLight => {
  const origin: ReferenceFramewUnits = node.origin ?? {};
  const position: Vector3wUnits = origin.position ?? Vector3wUnits.zero();
