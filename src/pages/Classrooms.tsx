...
export interface ClassroomRootRouteParams {
  classroomId: string;
  [key: string]: string;
...
export interface ClassroomsPublicProps extends StyleProps, ThemeProps {
  params: ClassroomRootRouteParams;
  classrooms: Dict<AsyncClassroom>;
  onCreateClassroom: (classroom: Classroom) => void;
  onLoadClassroom: () => void;
}

interface ClassroomsPrivateProps {
...
type Props = ClassroomsPublicProps & ClassroomsPrivateProps & WithNavigateProps;
type State = ClassroomsState;
...
const ClassroomsTitleContainer = styled('div', {
  alignItems: 'center',
  justifyContent: 'center',
  display: 'flex',
  flexDirection: 'column',
...
const ClassroomHeaderContainer = styled('div', {
  display: 'flex',
  flexDirection: 'row',
  // alignItems: 'center',
  justifyContent: 'center',
...
const ManageClassroomsContainer = styled('div', {
  display: 'flex',
  flexDirection: 'column',
  alignItems: 'stretch',
...
class Classrooms extends React.Component<Props, State> {
        ...
        />
        {
          showCreateClassroomDialog && (
            <CreateClassroomDialog
              ...
              onCloseClassroomDialog={function (classroomName: string): void {
                throw new Error('Function not implemented.');
              }}
              theme={DARK}
              locale={'en-US'}
              ...
              onUserCreation={function (userName: string): void {
                throw new Error('Function not implemented.');
              }}
            />
          )}
        ...
        <ClassroomsContainer style={style} theme={theme}>
          <ClassroomsTitleContainer>
            ...
            <ClassroomHeaderContainer>
              ...
              <Button theme={DARK}
                onClick={() => props.onCreateClassroom({
                  teacherId: { 'en-US': currentUserEmail },
                  name: { 'en-US': 'Test Classroom' },
                  code: { 'en-US': 'INVITE123' },
                  studentIds: []
...
const ConnectedClassroomRoot = connect(
  // --- mapStateToProps ---
  (state: ReduxState, { params: { classroomId } }: { params: ClassroomRootRouteParams }) => {
    const builder = new Builder(state);

    // Load classroom
    builder.classroom(classroomId);

    // Dispatch pending loads
    builder.dispatchLoads();

    return {
      classrooms: Dict.unique(builder.classrooms)[classroomId],
      locale: state.i18n.locale,
      theme: DARK,
    };
  },
  (dispatch, { params: { classroomId } }: { params: ClassroomRootRouteParams }) => ({
    onCreateClassroom: (classroom: Classroom) => {
      dispatch(ClassroomsAction.createClassroom({ classroomId, classroom }));
    },
    onLoadClassroom: () => {
  ...
  withNavigate(Classrooms)
) as React.ComponentType<{ params: ClassroomRootRouteParams }>;

export default withParams<ClassroomRootRouteParams>()(ConnectedClassroomRoot);